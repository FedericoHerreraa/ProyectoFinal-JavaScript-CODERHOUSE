...
// funcion para traer los productos en descuento con un fetch y mostrarlos en pantalla recorriendo el array
const mostrarProdDescontados = () => {
    fetch('/data/productosDescuento.json')
        .then(resp => resp.json())
        .then(dato => {
            dato.forEach(producto => {
                const divProdDescontados = document.createElement("div");
                divProdDescontados.innerHTML = `
                    <div class="card" id="cardDescuento" style="width: 18rem;">
                        <img src="${producto.imagen}" id="imagenProdDescuento" class="card-img-top" alt="...">
                        <div class="card-body" id="bodyDescuento">
                            <h5 class="card-text">${producto.producto}</h5>
                            <p class="card-text">Precio $${producto.precio}</p>
                        </div>
                    </div>
                `
                productosEnDescuento.append(divProdDescontados);
            })
        })
        .catch((e) => console.log(e));      
}
